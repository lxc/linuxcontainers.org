 ...
 * バックグラウンドでのイメージの同期がサーバでサポートされ、すべてのキャッシュされたイメージでデフォルトで同期されるようになりました <!-- Background image syncronization is now supported by the server and done by-default for all cached images -->
